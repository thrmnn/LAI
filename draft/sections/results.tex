...
The LAI values observed for the majority of trees (1–7) are consistent with previous investigations of urban canopies \cite{watts2018ReportLancet2018,faurieAssociationHighTemperature2022}, where LAI is typically lower than in natural forests. At these values, the canopy in tile 25BZ1 would be expected to intercept approximately $40–80\%$ of incoming shortwave radiation during the leaf-on season \cite{watts2018ReportLancet2018,faurieAssociationHighTemperature2022}. A multi-panel overview summarizing the structural and spectral characteristics of the tile is provided in Figure * 
...
\begin{figure}[h!]
    ...
    \label{fig:distribution_species}
\end{figure}

Finally, we look at the distribution of height, NDVI, and LAI separated by species. This is to verify that the proportions between species are in accordance with previous research. While the LAI values are slightly lower than generally expected, the proportions between the species from the Amsterdam database are in line with the consensus. In the case of height and NDVI, the same is true. While the mean height differences between species do not stray far from those expected, the difference in the top quantile of height for each species is more pronounced. This is in line with general factors such as tree age and pruning by the municipality, which lead to the small deviation in the mean \cite{kantorSmallscaleHumanbiometeorologicalImpacts2016}. 
